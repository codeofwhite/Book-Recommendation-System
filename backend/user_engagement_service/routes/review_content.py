...
def get_book_reviews(book_id):
    ...
    reviews = Review.query.filter_by(book_id=book_id, status=0).order_by(Review.post_time.desc()).all()
    # 返回字典列表，方便JSON序列化
...
def submit_review(book_id):
    ...
    if not all([user_id, content, rating is not None]):
        return jsonify({"error": "Missing required fields: userId, content, rating"}), 400
    if not (0 <= rating <= 5):
    ...
    try:
        db.session.commit()
        return jsonify({"message": "Review submitted successfully", "reviewId": new_review.review_id}), 201
...
def get_review_comments(review_id):
    comments = Comment.query.filter_by(review_id=review_id).order_by(Comment.comment_time.asc()).all()
...
def submit_comment(review_id):
    ...
    try:
        db.session.commit()
        return jsonify({"message": "Comment submitted successfully", "commentId": new_comment.comment_id}), 201
    except IntegrityError:
        db.session.rollback()
