...
class ReviewModel:
    ...
    @classmethod
    def get_all_reviews(cls, status_filter=None, search_keyword=None, page=1, per_page=10):
        """获取所有书评，支持筛选、搜索和分页"""
        try:
            query = Review.query
...
class CommentModel:
    ...
    def get_comments_by_review(cls, review_id, page=1, per_page=10):
        """根据书评ID获取评论"""
        try:
            query = Comment.query.filter_by(review_id=review_id).order_by(Comment.comment_time.asc())
            ...
            for comment in pagination.items:
                comment_dict = {
                    ...
                    'commentTime': comment.comment_time.isoformat() if comment.comment_time else None
                }
