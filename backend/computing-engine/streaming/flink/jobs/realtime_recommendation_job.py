...
class FlinkJobConfig:
    ...
    REDIS_RECENT_VIEWS_HASH_KEY = (
        "user_recent_views_hash:"  # 保持与 config.py 一致，使用 HASH KEY
    )
    ...
    # 物品特征向量前缀
    REDIS_BOOK_FEATURES_PREFIX = os.getenv(
        "REDIS_BOOK_FEATURES_PREFIX", "book_features:"
    )
    # 用户画像向量前缀
    ...
    RECOMMENDATION_CACHE_EXPIRE_SECONDS = int(
        os.getenv("RECOMMENDATION_CACHE_EXPIRE_SECONDS", 3600 * 24)
    )
    # 每个用户最多保留的最近浏览记录数量
    ...
    REALTIME_CF_BOOST_DWELL_TIME_THRESHOLD = int(
        os.getenv("REALTIME_CF_BOOST_DWELL_TIME_THRESHOLD", 30)
    )
    ...
    REALTIME_CONTENT_FUSION_ALPHA = float(
        os.getenv("REALTIME_CONTENT_FUSION_ALPHA", 0.7)
    )  # 默认值0.7，给用户画像更多权重
...
class JsonParser(FlatMapFunction):
    ...
    def flat_map(self, json_str: str):
        try:
            ...
            logger.warning(
                f"Skipping record: Failed to decode JSON string: '{json_str}'. Error: {e}"
            )
        except (ValueError, TypeError, AttributeError) as e:
            logger.warning(
                f"Skipping record due to data format inconsistency or type error: '{json_str}'. Error: {e}"
            )
        except Exception as e:
            logger.error(
                f"An unexpected error occurred while processing JSON string: '{json_str}'. Error: {e}",
                exc_info=True,
            )
...
class UserRecentViewsProcessFunction(KeyedProcessFunction):
    ...
    def open(self, runtime_context):
        ...
        try:
            ...
            logger.info(
                f"[{runtime_context.get_task_name()}] Successfully connected to Redis at {FlinkJobConfig.REDIS_HOST}:{FlinkJobConfig.REDIS_PORT}!"
            )
        except Exception as e:
            logger.error(
                f"[{runtime_context.get_task_name()}] Error connecting to Redis at {FlinkJobConfig.REDIS_HOST}:{FlinkJobConfig.REDIS_PORT}: {e}"
            )
            self.redis_client = None
        ...
        self.recent_views_state = runtime_context.get_state(
            ValueStateDescriptor("recent_views_list_state", Types.PICKLED_BYTE_ARRAY())
        )
        ...
        if user_profile_bytes:
            try:
                ...
                if isinstance(profile_vector, list):
                    ...
                    logger.warning(
                        f"User profile for {user_id} is not a list after JSON decoding. Type: {type(profile_vector)}"
                    )
                    return None
            except (json.JSONDecodeError, TypeError) as e:
                logger.error(
                    f"Failed to decode user profile JSON for {user_id}: {e}",
                    exc_info=True,
                )
                return None
        else:
        ...
        if book_features_bytes:
            try:
                ...
                if isinstance(feature_vector, list):
                    ...
                    logger.warning(
                        f"Book features for {book_id} is not a list after JSON decoding. Type: {type(feature_vector)}"
                    )
                    return None
            except (json.JSONDecodeError, TypeError) as e:
                logger.error(
                    f"Failed to decode book features JSON for {book_id}: {e}",
                    exc_info=True,
                )
                return None
        else:
    ...
    def process_element(self, value: Row, ctx: "KeyedProcessFunction.Context"):
        ...
        try:
            ...
            if current_views_pickled is not None:
                try:
                    ...
                    if not isinstance(current_views_list, list):
                        logger.warning(
                            f"Recent views state for user {user_id} is not a list after deserialization. Resetting."
                        )
                        current_views_list = []
                except (pickle.UnpicklingError, json.JSONDecodeError, TypeError) as e:
                    logger.error(
                        f"Failed to deserialize recent views state for user {user_id}: {e}. Resetting state.",
                        exc_info=True,
                    )
                    current_views_list = []
            ...
            current_views_list = [
                view
                for view in current_views_list
                if view.get("item_id") != current_item_id
            ]  # 移除旧的相同 item_id
            current_views_list.append(new_view)
            current_views_list = sorted(
                current_views_list, key=lambda x: x.get("timestamp", 0), reverse=True
            )[: self.MAX_RECENT_VIEWS]

            # 更新 Flink 状态
            self.recent_views_state.update(
                pickle.dumps(json.dumps(current_views_list, ensure_ascii=False))
            )
            ...
            self.redis_client.hset(
                ...
                json.dumps(
                    current_views_list, ensure_ascii=False
                ),  # Redis Hash 的值是最近浏览列表的 JSON 字符串
            )
            logger.info(
                f"[{ctx.get_current_key()}] Updated recent views for user {user_id}. Current views count: {len(current_views_list)}"
            )

            # --- 3. 从 Redis 获取离线推荐结果 ---
            offline_recs_key = (
                f"{FlinkJobConfig.OFFLINE_RECOMMENDATIONS_KEY_PREFIX}{user_id}"
            )
            ...
            if offline_recs_json:
                try:
                    ...
                    logger.error(
                        f"Failed to decode offline recommendations JSON for user {user_id}: {e}. Raw: {offline_recs_json}",
                        exc_info=True,
                    )
                except Exception as e:
                    logger.error(
                        f"Error processing offline recommendations for user {user_id}: {e}",
                        exc_info=True,
                    )
            else:
                logger.info(
                    f"No offline recommendations found for user {user_id} at key {offline_recs_key}. Using empty list."
                )
            ...
            for rec_item in offline_recommendations:
                rec_item_id = rec_item.get("book_id") or rec_item.get(
                    "item_id"
                )  # 适配不同的命名
            ...
            if (
                ...
                >= FlinkJobConfig.REALTIME_CF_BOOST_DWELL_TIME_THRESHOLD
            ):
                realtime_cf_weight = min(
                    1.0,
                    FlinkJobConfig.OFFLINE_CF_WEIGHT
                    + FlinkJobConfig.REALTIME_CF_BOOST_AMOUNT,
                )
            ...
            for rec_item in filtered_offline_recs:
                rec_book_id = rec_item.get("book_id") or rec_item.get(
                    "item_id"
                )  # 确保使用正确的 ID
                ...
                offline_mixed_score = rec_item.get(
                    "score",
                    (realtime_cf_weight * raw_cf)
                    + (realtime_popularity_weight * raw_pop),
                )
                try:
                ...
                if user_profile_vec is not None and rec_book_id is not None:
                    rec_book_feature_vec_for_profile = self._get_book_feature_vector(
                        rec_book_id
                    )
                ...
                if (
                    current_item_id
                    and current_book_feature_vec is not None
                    and rec_book_id is not None
                ):
                    rec_book_feature_vec_for_last_clicked = (
                        self._get_book_feature_vector(rec_book_id)
                    )
                    if rec_book_feature_vec_for_last_clicked is not None:
                        last_clicked_item_content_similarity = self._cosine_similarity(
                ...
                combined_content_score = (
                    FlinkJobConfig.REALTIME_CONTENT_FUSION_ALPHA
                    * user_profile_content_similarity
                ...
                rec_item["realtime_score"] = (
                    FlinkJobConfig.SIMILARITY_FUSION_ALPHA * combined_content_score
                ) + (
            ...
            final_recommendations = sorted(
                filtered_offline_recs,
                key=lambda x: x.get("realtime_score", 0.0),
                reverse=True,
            )
            final_recommendations = final_recommendations[
                : self.FINAL_RECOMMENDATION_COUNT
            ]
            ...
            self.redis_client.setex(
                ...
                json.dumps(final_recommendations, ensure_ascii=False).encode(
                    "utf-8"
                ),  # Redis 存储为字节，确保非 ASCII 字符正确编码
            )
            logger.info(
                f"[{ctx.get_current_key()}] Stored {len(final_recommendations)} real-time updated recommendations for user {user_id} at key: {final_recs_redis_key}."
            )

        except Exception as e:
            logger.error(
                f"Error processing element {value} for key {ctx.get_current_key()}: {e}",
                exc_info=True,
            )
...
def main():
    ...
    json_row_type_info = Types.ROW_NAMED(
        ...
        [
            ...
            Types.STRING(),
            Types.STRING(),
            Types.ROW_NAMED(["pageName", "dwellTime"], [Types.STRING(), Types.INT()]),
        ],
    )
    ...
    logger.info(
        "Starting PyFlink Realtime User Recent Views Aggregation and Hybrid Recommendation Job..."
    )
    # 执行 Flink 作业，并指定作业名称
    env.execute(
        "PyFlink Realtime User Recent Views Aggregator and Hybrid Recommendation"
    )
